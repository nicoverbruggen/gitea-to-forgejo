...
from .features import activity, avatars, issues, mirrors, organizations, packages, releases, repositories, repo_units, social, ssh, users
from .helpers.common import SUPPORTED_ACTIVITY_OP_TYPES, ValidationFailure, normalize_int, normalize_text, sample_values
from .helpers.files import sha256_file
from .helpers.git import git_fsck_output, git_head_text, git_ref_map
...
class Validator:
    def __init__(
        ...
        source_packages = self.fetch_all(self.source, "select * from package order by id")
        source_versions = self.fetch_all(self.source, "select * from package_version order by id")
        source_files = self.fetch_all(self.source, "select * from package_file order by id")
        source_blobs = self.fetch_all(self.source, "select * from package_blob order by id")
        source_properties = self.fetch_all(self.source, "select * from package_property order by id")
        (
        ...
        self.pruned_source_package_version_count = len(source_versions) - len(self.retained_source_package_versions)
        self.pruned_source_package_file_count = len(source_files) - len(self.retained_source_package_files)
        self.pruned_source_package_blob_count = len(source_blobs) - len(self.retained_source_package_blobs)
    ...
    def compare_values(self, check: str, label: str, source_value: Any, target_value: Any) -> None:
        if source_value != target_value:
            self.add_failure(check, f"{label}: expected {source_value!r}, found {target_value!r}")

    def compare_key_sets(self, check: str, source_keys: set[Any], target_keys: set[Any]) -> None:
    ...
    def compare_entity(self, check: str, entity: str, source_rows: dict[int, tuple[Any, ...]], target_rows: dict[int, tuple[Any, ...]]) -> None:
    ...
    def target_repo_key_map(self) -> dict[int, tuple[str, str]]:
        return {
            row["id"]: (row["owner_name"], row["lower_name"])
    ...
    def git_fsck(self, repo_path: Path) -> None:
        returncode, detail = git_fsck_output(repo_path)
        if returncode != 0:
            self.add_failure("git-fsck", f"{repo_path}: {detail or 'git fsck failed without output'}")

    def run(self) -> int:
        self.validate_database()
        users.validate(self)
        organizations.validate(self)
        repositories.validate(self)
        repo_units.validate(self)
        issues.validate(self)
        releases.validate(self)
        social.validate(self)
        ssh.validate(self)
        avatars.validate(self)
        mirrors.validate(self)
        packages.validate(self)
        activity.validate(self)
        self.write_report()
    ...
    def validate_org_memberships(self) -> None:
        ...
        source_rows = {
            (row["org_name"], row["member_name"], normalize_int(row["is_public"]))
            for row in self.fetch_all(
                self.source,
                """
    ...
    def validate_repositories(self) -> None:
        ...
        source_repo_by_id = self.source_repo_key_map()
        target_repo_by_id = self.target_repo_key_map()
    ...
    def validate_repo_units(self) -> None:
        check = "repo-units"
        source_repo_keys = self.source_repo_key_map()
        target_repo_keys = self.target_repo_key_map()
    ...
    def validate_issues(self) -> None:
        check = "issues"
        source_user_names = self.source_user_name_map()
        target_user_names = self.target_user_name_map()
        source_repo_keys = self.source_repo_key_map()
        target_repo_keys = self.target_repo_key_map()
        ...
        self.compare_entity(check, "labels", source_labels, target_labels)
        ...
        self.compare_entity(check, "milestones", source_milestones, target_milestones)
        ...
        self.compare_entity(check, "issues", source_issues, target_issues)
        ...
        self.compare_entity(check, "issue-labels", source_issue_labels, target_issue_labels)
        ...
        self.compare_entity(check, "issue-assignees", source_issue_assignees, target_issue_assignees)
        ...
        self.compare_entity(check, "issue-users", source_issue_users, target_issue_users)
        ...
        self.compare_entity(check, "issue-watches", source_issue_watches, target_issue_watches)
        ...
        self.compare_entity(check, "comments", source_comments, target_comments)
        ...
        self.compare_entity(check, "pull-requests", source_pull_requests, target_pull_requests)

        source_reviews = {
            row["id"]: (
                normalize_int(row["type"]),
                source_user_names.get(normalize_int(row["reviewer_id"]), ""),
                normalize_int(row["reviewer_team_id"]),
                normalize_text(row["original_author"]),
                source_user_names.get(normalize_int(row["original_author_id"]), ""),
                normalize_int(row["issue_id"]),
                normalize_text(row["content"]),
                normalize_int(row["official"]),
                normalize_text(row["commit_id"]),
                normalize_int(row["stale"]),
                normalize_int(row["dismissed"]),
                normalize_int(row["created_unix"]),
                normalize_int(row["updated_unix"]),
            )
            for row in self.fetch_all(self.source, "select * from review order by id")
        }
        target_reviews = {
            row["id"]: (
                normalize_int(row["type"]),
                target_user_names.get(normalize_int(row["reviewer_id"]), ""),
                normalize_int(row["reviewer_team_id"]),
                normalize_text(row["original_author"]),
                target_user_names.get(normalize_int(row["original_author_id"]), ""),
                normalize_int(row["issue_id"]),
            ...
            for row in self.fetch_all(self.target, "select * from review order by id")
        }
        self.compare_entity(check, "reviews", source_reviews, target_reviews)

        source_review_states = {
            row["id"]: (
                source_user_names.get(normalize_int(row["user_id"]), ""),
                normalize_int(row["pull_id"]),
                normalize_text(row["commit_sha"]),
                normalize_int(row["updated_files"]),
                normalize_int(row["updated_unix"]),
            )
            for row in self.fetch_all(self.source, "select * from review_state order by id")
        }
        target_review_states = {
            ...
            for row in self.fetch_all(self.target, "select * from review_state order by id")
        }
        self.compare_entity(check, "review-states", source_review_states, target_review_states)
        ...
        self.compare_entity(check, "issue-history", source_issue_history, target_issue_history)
        ...
        self.compare_entity(check, "reactions", source_reactions, target_reactions)
        ...
        self.add_note(
            ...
            f"{len(source_reviews)} reviews, "
            f"{len(source_review_states)} review states, "
            f"{len(source_reactions)} reactions, and "
            f"{len(source_issue_watches)} issue watches"
        )

    def validate_releases(self) -> None:
        check = "releases"
        source_user_names = self.source_user_name_map()
        target_user_names = self.target_user_name_map()
        source_repo_keys = self.source_repo_key_map()
        ...
        self.compare_entity(check, "release-rows", source_releases, target_releases)
        ...
        self.compare_entity(check, "uploads", source_uploads, target_uploads)

        self.add_note(f"Validated {len(source_releases)} releases and {len(source_uploads)} uploads")
    ...
    def validate_social(self) -> None:
        ...
        self.compare_entity(check, "attachments", source_attachments, target_attachments)
        ...
        self.compare_entity(check, "notifications", source_notifications, target_notifications)
        ...
        self.compare_entity(check, "stars", source_stars, target_stars)
        ...
        self.compare_entity(check, "follows", source_follows, target_follows)
        ...
        self.compare_entity(check, "watches", source_watches, target_watches)
        ...
        self.compare_entity(check, "collaboration", source_collaboration, target_collaboration)
    ...
    def validate_git_repositories(self) -> None:
        ...
        source_rows = self.fetch_all(
            self.source,
            "select owner_name, lower_name, name from repository order by owner_name, lower_name",
        )
        validated = 0
        total_refs = 0
        ...
        for row in source_rows:
            ...
            self.compare_values(check, f"{repo_label}.HEAD", git_head_text(source_path), git_head_text(target_path))

            try:
                source_refs = git_ref_map(source_path)
                target_refs = git_ref_map(target_path)
            except subprocess.CalledProcessError as exc:
                detail = exc.stderr.strip() or exc.stdout.strip()
                self.add_failure(check, f"{repo_label}: failed to enumerate refs: {detail}")
                continue
            ...
            if source_refs != target_refs:
                ...
                if mismatched_refs:
                    ...
                    detail = ", ".join(
                        f"{ref_name} ({source_refs[ref_name]} != {target_refs[ref_name]})"
                        for ref_name in sample
                    )
    ...
    def validate_avatars(self) -> None:
        ...
        for row in self.fetch_all(
            self.source,
            "select name, type, avatar, use_custom_avatar from user order by id",
        ):
            ...
            self.compare_file_contents(check, f"user-avatar:{row['name']}", source_path, target_path)
            validated += 1
        ...
        for row in self.fetch_all(self.source, "select owner_name, name, avatar from repository order by id"):
            ...
            self.compare_file_contents(check, f"repo-avatar:{row['owner_name']}/{row['name']}", source_path, target_path)
            validated += 1
    ...
    def validate_pull_mirrors(self) -> None:
        ...
        expected_rows = {
            (row["owner_name"], row["lower_name"]): row
            for row in source_rows
            if (normalize_text(row["owner_name"]).lower(), normalize_text(row["name"]).lower())
            not in self.warning_repo_keys
        }
        target_rows = {
            (row["owner_name"], row["lower_name"]): row
            for row in self.fetch_all(
    ...
    def validate_packages(self) -> None:
        ...
        for package_key in sorted(set(source_packages) & set(target_packages)):
            ...
            self.compare_values(
                ...
                f"{label}.repo",
                (
                    normalize_text(source_row["repo_owner_name"]),
                    normalize_text(source_row["repo_lower_name"]),
                ),
                (
                    normalize_text(target_row["repo_owner_name"]),
                    normalize_text(target_row["repo_lower_name"]),
                ),
            )

        retained_version_ids = {row["id"] for row in self.retained_source_package_versions}
        source_versions = {
            (row["owner_name"], row["package_name"], row["version"]): row
            for row in self.fetch_all(
                self.source,
                """
            ...
            if row["id"] in retained_version_ids
        }
        target_versions = {
            (row["owner_name"], row["package_name"], row["version"]): row
            for row in self.fetch_all(
        ...
        retained_file_ids = {row["id"] for row in self.retained_source_package_files}
        source_files = {
            (row["owner_name"], row["package_name"], row["version"], row["name"]): row
            for row in self.fetch_all(
                self.source,
                """
                select package_file.*,
            ...
            if row["id"] in retained_file_ids
        }
        target_files = {
            (row["owner_name"], row["package_name"], row["version"], row["name"]): row
            for row in self.fetch_all(
        ...
        for file_key in sorted(set(source_files) & set(target_files)):
            ...
            self.compare_values(
                check,
                f"{label}.version_id",
                version_id_map[source_row["version_id"]],
                target_row["version_id"],
            )
            self.compare_values(
                check,
                f"{label}.blob_id",
                blob_id_map[source_row["blob_id"]],
                target_row["blob_id"],
            )
            for field in ("lower_name", "composite_key", "is_lead", "created_unix"):
                self.compare_values(check, f"{label}.{field}", source_row[field], target_row[field])

        retained_property_ids = {row["id"] for row in self.retained_source_package_properties}
        source_properties = {
            (
            ...
            if row["id"] in retained_property_ids
        }
        target_properties = {
            (
                normalize_int(row["ref_type"]),
